...
namespace ProblemSet_01_SortingAndSearching
{
    ...
    public static class SortAndSearchExtensions
    {
        ...
        public static IList<T> SelectionSort<T>(this IList<T> listToSort, CompareDelegate<T> comparer)
        {
            for (int i = 0; i < listToSort.Count; i++)
            {
                int minItemPos = i;
                for (int j = i + 1; j < listToSort.Count; j++)
                {
                    if (comparer(listToSort[minItemPos], listToSort[j]) > 0)
                    {
                        minItemPos = j;
                    }
                }
        ...
        public static int BSearch<T>(this IList<T> listForBS, T key, CompareDelegate<T> comparer)
        {
            int minIndex = 0;
            int maxIndex = listForBS.Count - 1;
            while (minIndex <= maxIndex)
            {
                ...
                if (comparer(listForBS[midIndex], key) == 0) return midIndex;
                if (comparer(listForBS[midIndex], key) < 0) minIndex = midIndex + 1;
                else maxIndex = midIndex - 1;
        ...
        public static IList<T> QuickSort<T>(this IList<T> listToSort, CompareDelegate<T> comparer)
        {
            QuickSortRecursion(listToSort, 0, listToSort.Count - 1, comparer);
            return listToSort;
        }
        ...
        private static void QuickSortRecursion<T>(IList<T> listToSort, int leftIndex, int rightIndex, CompareDelegate<T> comparer)
        {
            if (leftIndex < rightIndex)
            {
                int partitionPlace = QuickSortPartition(listToSort, leftIndex, rightIndex, comparer);
        ...
        private static int QuickSortPartition<T>(IList<T> listToSort, int leftIndex, int rightIndex, CompareDelegate<T> comparer)
        {
            T pivot = listToSort[rightIndex];
            int indexToSwap = leftIndex;
            for (int j = leftIndex; j < rightIndex; j++)
            {
                if (comparer(listToSort[j], pivot) <= 0)
                {
                    T temp = listToSort[indexToSwap];
                    listToSort[indexToSwap] = listToSort[j];
                    listToSort[j] = temp;
        ...
        public static IList<T> MergeSort1<T>(this IList<T> listToSort, CompareDelegate<T> comparer)
        {
            var linked = new LinkedList<T>();
            foreach (T item in listToSort)
            {
        ...
        private static LinkedList<T> MergeSortLinkedListRecursion<T>(LinkedList<T> list, CompareDelegate<T> comparer)
        {
            if (list.Count <= 1) return list;
        ...
        private static LinkedList<T> MergeSortLinkedListMerge<T>(LinkedList<T> left, LinkedList<T> right, CompareDelegate<T> comparer)
        {
            var merged = new LinkedList<T>();
            while (left.Count != 0 && right.Count != 0)
            {
                if (comparer(left.First.Value, right.First.Value) <= 0)
                {
                    merged.AddLast(left.First.Value);
                    left.RemoveFirst();
                }
